...
fn module_add_and_remove_tear_down_as_a_unit() {
    let mut pm = Pm::new();
    let outside = pm.pool_get::<Pos>("outside");
    ...
    assert_eq!(*runs.borrow(), 1);
    let fresh = pm.pool_get::<Pos>("mod_pos");
    assert_eq!(fresh.borrow().len(), 0);
    assert!(Rc::ptr_eq(&outside, &pm.pool_get::<Pos>("outside")));
}
...
fn module_init_error_rolls_back_registration() {
    ...
    let pool = pm.pool_get::<Pos>("broken_pool");
    let id = pm.id_add();
