...
struct ServerState {
    ...
    void reset_game(TaskContext& ctx, NetSys* net, Pool<Monster>* mp, Pool<Bullet>* bp) {
...
void server_init(Pm& pm) {
    ...
    // --- Packet handlers ---
    net->on_recv(PKT_INPUT, [gs, net](TaskContext&, const uint8_t* buf, int n, struct sockaddr_in&) {
    ...
    net->on_recv(PKT_PAUSE, [](TaskContext& ctx, const uint8_t*, int, struct sockaddr_in&) {
        ctx.toggle_pause();
    });

    net->on_recv(PKT_RESTART, [gs, net, mp, bp](TaskContext& ctx, const uint8_t*, int, struct sockaddr_in&) {
        if (!gs->game_over) return;
        gs->reset_game(ctx, net, mp, bp);
        ctx.resume();
        printf("[server] game restarted\n");
    });

    // --- Player movement + shooting ---
    pm.schedule("player_move", Phase::SIMULATE - 1.f, [gs, bp](TaskContext& ctx) {
    ...
    // --- Status print ---
    pm.schedule("status", Phase::HUD, [gs, mp, bp](TaskContext& ctx) {
