...
//
// Depends on: pm_sdl.hpp (SdlSystem), SDL2_image (-lSDL2_image)
//
// Usage:
//   IMG_Init(IMG_INIT_PNG);  // call once at startup
//   Sprite s;
...
struct Sprite
{
    ...
    {
        if (!tex) return;
        float display_h = (w > 0) ? display_w * ((float)h / (float)w) : display_w;
        SDL_Rect dst = {
            (int)(cx - display_w  * 0.5f),
            (int)(cy - display_h * 0.5f),
            (int)display_w,
            (int)display_h
        };
