...
pub fn run(quiet: bool) {
    ...
    task!(pm, "respawn", 11.0, [car, net], move |_pm| {
        for (peer, _) in respawns.drain() {
            if let Some(id) = net.own(peer)
                && let Some(mut c) = car.get_id_mut(id)
            {
                *c = spawn_car(peer);
            }
        }
    ...
    task!(pm, "roster", 10.0, [car, score, net], move |pm| {
        for p in net.joined() {
            let id = pm.id_add();
    ...
    task!(pm, "score", 31.0, [car, score, contact, net], move |pm| {
        ...
        for (peer, id) in net.owned() {
            let Some(me) = car.get_id(id).map(|c| *c) else {
                continue;
            };
            ...
            let hit_at = others.iter().find_map(|o| {
                capsule_overlap(&me, o).map(|_| ((me.x + o.x) * 0.5, (me.z + o.z) * 0.5))
            });
    ...
    if !quiet {
        task!(pm, "status", 90.0, 5.0, [car, score, contact], move |pm| {
            let cars = car.get();
