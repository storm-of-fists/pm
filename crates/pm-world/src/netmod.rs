...
impl PmClient {
    ...
    /// game's correction threshold over it — a CLOSURE, read at every
    /// reconcile, so a params-driven dead zone tunes live. Errors at
    /// or under it are accepted drift (prediction keeps its own
    /// state); above it the rewind-replay runs. Pair with
